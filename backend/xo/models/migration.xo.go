...
// Migration represents a row from 'demo.migrations'.
...
func (m *Migration) Insert(db XODB) error {
	...
	const sqlstr = `INSERT INTO demo.migrations (` +
		`id, applied_at` +
		`) VALUES (` +
		`?, ?` +
		`)`
...
func (m *Migration) Update(db XODB) error {
	...
	// sql query
	const sqlstr = `UPDATE demo.migrations SET ` +
		`applied_at = ?` +
		` WHERE id = ?`
...
func (m *Migration) Delete(db XODB) error {
	...
	// sql query
	const sqlstr = `DELETE FROM demo.migrations WHERE id = ?`
...
// MigrationByID retrieves a row from 'demo.migrations' as a Migration.
//
