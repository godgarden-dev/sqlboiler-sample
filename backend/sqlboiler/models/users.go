...
var (
	userColumns               = []string{"id", "email", "display_name", "created_at", "updated_at"}
	userColumnsWithoutDefault = []string{"email", "display_name", "created_at", "updated_at"}
...
func (userL) LoadUserProfile(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(qm.From(`user_profiles`), qm.WhereIn(`user_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
